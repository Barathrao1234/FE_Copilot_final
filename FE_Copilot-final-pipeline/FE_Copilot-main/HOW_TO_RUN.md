...
4. Update `sources/source_manifest.json` so every source file is classified
    ...
    `standards`, `migrated_query`), seam participation is explicit, and
...
all outside bytes. Doc 08 is retired as a separate file — the `/08-ai-agent-instruction`
command no longer exists; run `/07-quality-and-operations` to get Part F instead.
...
## 5. STANDING RULES (carry everything)
   ...
   R3. spec_repo/ is never hand-edited — targeted regeneration only; IDs are
       stable forever.
   R4. Any change to sources/ is a ground-truth event: preflight + regenerate
       the affected cascade FIRST — this includes a revised reverse-engineering
       document; edit/replace the file in sources/ directly and regenerate like
       any other ground-truth event. Source-role/seam changes must be captured
       in source_manifest.json in the
       same commit.
...
CONNECTION DAY (schedule a HALF-DAY working session, not a checkbox):
   ...
   b. If the client also hands over their own data model/DDL: STOP — that is
      the external-schema mode; the DDL goes into sources/ classified
      `authoritative_data_model`, doc 02 is regenerated against it (`/00-preflight`
      then targeted or full regen per the change scope), and their schema becomes
      the authority.
