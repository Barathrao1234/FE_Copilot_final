...
for ($idx = 0; $idx -lt $questions.Count; $idx++) {
    ...
    Add-Content -Path $DecisionFile -Value "- Question: $($q.Text)"
    Add-Content -Path $DecisionFile -Value "- Decision: $decision"
    Add-Content -Path $DecisionFile -Value "- Applies to: $appliesTo"
    if (-not [string]::IsNullOrWhiteSpace($Stage)) {
        Add-Content -Path $DecisionFile -Value "- Applies to stages: $Stage"
    }
    Add-Content -Path $DecisionFile -Value "- Source basis: $sourceBasis"
    Add-Content -Path $DecisionFile -Value "- References: $($q.File):$($q.Line)"
    Add-Content -Path $DecisionFile -Value "- Notes: $notes"

    $recorded++

    Write-Host "Recorded $decisionId" -ForegroundColor Green
    Write-Host ""
}
