...
def main() -> int:
    ...
    phase_map: dict[str, set[str]] = defaultdict(set)
    phase_rx = re.compile(r"\bT-(\d+)\.\d+\b")
    for spec in index["specs"]:
        if not spec["path"].endswith("06_Build_Plan.md"):
            continue
        for row in spec["tracked_rows"]:
            req_ids = {rid for rid in ID_RX.findall(row["text"]) if rid.startswith(("FR-", "BR-", "NFR-"))}
            task_ids = phase_rx.findall(row["text"])
            if req_ids and task_ids:
                for req_id in req_ids:
                    for phase in task_ids:
                        phase_map[phase].add(req_id)

    phase_lines = [
        "# Derived Phase Coverage Map",
        "",
        "Generated mechanically from doc 06 B4 (requirement -> covering task IDs) "
        "joined with each task ID's own phase prefix (T-<phase>.<seq>). Doc 06 A4 "
        "does not author this table by hand; this file is the authority for it.",
        "",
        "| Phase | FR/BR/NFR IDs delivered |",
        "| --- | --- |",
    ]
    for phase in sorted(phase_map, key=int):
        phase_lines.append(f"| {phase} | {', '.join(sorted(phase_map[phase]))} |")
    write("phase_coverage.md", "\n".join(phase_lines))

    print(f"Derived views: PASS ({len(locations)} IDs, {len(obligations)} obligations, "
        f"{len(questions)} questions, {len(phase_map)} phases mapped)")
    return 0
