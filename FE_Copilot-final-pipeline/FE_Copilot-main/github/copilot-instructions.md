...
- **Ground-truth events:** any addition, removal, or modification of a file in
  `sources/` (other than change_requests.md) is a ground-truth event — record it
  (CR or new-baseline note), run `/00-preflight`, and regenerate the affected
  cascade before any other pipeline activity. A revision of the reverse docs
  against an EXISTING pack uses the reconciliation workflow instead:
  revised docs in `sources_new/` → reconciliation audit → RECONCILIATION.md →
  `scripts/verify_reconciliation.py` (completeness gate) → then either rebase the
  pack (bake the audit into doc 01 as a Source Baseline appendix, retarget all
  citations, promote the new docs into `sources/`, remove the staging folder) or
  file CRs for the MISSING/CONFLICT rows.
...
- **Regeneration mode is explicit:** `FULL_BASELINE` for first generation,
  structural changes, three or more batched CRs, or stale/failed indexes;
...
## Change requests (token-efficient regeneration)

All requirement adds/edits/deletes enter via `sources/change_requests.md` with a
completed Impact Analysis. Small additive CR → targeted regeneration ("update ONLY
the affected parts, preserve IDs, change nothing else"), then `/verify`. Structural
CR or 3+ batched CRs → full cascade at a phase boundary (behavior-only:
01→05→06→07; data adds 02+04; API shape adds 04). Withdrawn keeps the ID.
...
F — /08 no longer exists as a command; use \/07` instead) → `/verify` (DELTA`A
