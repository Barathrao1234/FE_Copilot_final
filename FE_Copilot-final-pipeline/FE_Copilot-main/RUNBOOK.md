...
`/08-ai-agent-instruction` no longer exists — run `/07-quality-and-operations`
instead (or re-run it if your existing pack predates this change) to get Part F.
...
Rework of an individual task: re-run `/run-phase` for that task's phase; the
progress ledger (`PROGRESS.md`) marks already-DONE tasks so only the reworked
task and any downstream tasks in that phase are re-executed.
...
Edit the affected source file(s) directly (look up impact via doc 07 C1's
Forward Trace) → `/00-preflight` → targeted regeneration or full cascade per
`.github/copilot-instructions.md`'s policy → `/verify` → re-execute affected
tasks via `/run-phase` for the phase(s) containing them.

This also covers a revised reverse-engineering document replacing an existing
one: edit/replace the file in `sources/` directly, then follow the same flow —
there is no separate staging step.
